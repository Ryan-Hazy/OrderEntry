...
1. Go to the "Attachments" folder and find all of the images and move them to the "Images" folder
2. Click the run button which is the green play button located in the top right corner
3. At the end of the program running it will spit out a random number and this number will be at the end of the current date on the file
name
4. On the side bar on the left there will be a number of different files, to open the output that just ran, find the file name as
described above
5. Once the file is open drag down the formulas in cells L2 and S2
6. Filter on the "Check" collumn and show "ERROR" and go back to either the OE Files or the emails and find the correct information then
delete the contents of the cell that says "ERROR"
7. Go into the Images tab and enter the neccesary information into the output file
8. Move the contents of the "Attachments" folder to the "Old Attachments" folder
9. Move the contents of the "Images" folder to the "Old Images" folder
...
for i in range(len(entries)):
   ...
   for i in range(41):
       ...
       category = oews["L"+ix].value
       if(category == None):
           category = oews["K"+ix].value
           if(category == None):
               category = oews["J"+ix].value
               if(category == None):
